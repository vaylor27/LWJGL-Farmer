...
void main(void) {
	...
	float refractiveVector = dot(viewVector, normal);
	refractiveVector = pow(refractiveVector, 2);
	...
	reflectColor += vec4(specularHighlights, 0.0);
	out_Color = mix(reflectColor, refractColor, refractiveVector);
